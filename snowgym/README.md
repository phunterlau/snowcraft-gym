...
simulator. The current scaffold provides:
...
- a deterministic masked entity behavior-cloning trainer; and
- versioned checkpoint loading, closed-loop evaluation, and replay recording.
...
seeds, state hashes, versions, and result digest. The first committed learned
checkpoint also won both held-out seeds in 54 decisions with zero rejected
actions, versus the teacher's 53. See the
[training guide](./training/README.md) and
