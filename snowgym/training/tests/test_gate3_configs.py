...
def test_committed_gate3_ppo_series_is_auditable_and_qualifies() -> None:
    training_root = Path(__file__).resolve().parents[1]
    audit = audit_ppo_series(training_root / "runs/ppo_3v3_random_bc_v0")
    assert audit["mode"] == "qualifying"
    assert audit["gate"] == "3v3-random"
