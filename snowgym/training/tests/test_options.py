...
from snowgym_training.options.r1e import load_r1e_config
...
def test_frozen_engage_r1e_config_only_extends_r1d_duration() -> None:
    r1d = load_r1d_config()
    r1e = load_r1e_config()
    assert r1e["sourceUpdate"] == 100
    assert r1e["retainedUpdates"] == [100, 150, 200]
    assert r1e["finalUpdate"] == 200
    for name in (
        "option", "intervention", "bcAnchorFloor", "selectionPolicy",
        "reservoirBcFraction", "reservoirSeedPartition", "reservoirSeedCount",
        "worlds", "rolloutSteps", "anchorTotalUpdates", "trainingSeed",
        "ppoConfig", "bcLossConfig", "bootstrapGates",
    ):
        assert r1e[name] == r1d[name]
...
def test_failed_engage_r1c_candidate_passes_digest_audit() -> None:
    root = (
        Path(__file__).resolve().parents[1]
    ...
    assert manifest["trainingRuns"]["50"]["stateDigest"] == (
        manifest["config"]["referenceUpdate50StateDigest"]
    )
    assert trajectory["summaries"]["100"]["correct"]["contactRate"] == 0.65
    assert trajectory["summaries"]["100"]["correct"]["hitRate"] == 0.025
    assert not bootstrap["passed"]
    assert not manifest["passed"]


def test_engage_r1d_candidate_passes_digest_audit() -> None:
    root = (
        Path(__file__).resolve().parents[1]
    ...
    for update in (50, 75, 100):
        ...
        assert metadata["collectorConfig"]["bcAnchorFloor"] == 0.05
    correct = trajectory["summaries"]["100"]["correct"]
    assert correct["contactRate"] == 0.85
    assert correct["hitRate"] == 0.525
