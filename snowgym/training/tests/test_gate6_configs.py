...
def test_committed_gate6_ppo_series_is_auditable_and_qualifies() -> None:
    training_root = Path(__file__).parents[1]
    audit = audit_ppo_series(training_root / "runs/ppo_5v5_terrain_bc_v0")
    assert audit["mode"] == "qualifying"
    assert audit["gate"] == "5v5-random-terrain"
    ...
    for update in (1, 5, 10):
        evaluation = json.loads(
            (
                training_root
                / f"runs/ppo_5v5_terrain_bc_v0/evaluations/update-{update:06d}.json"
