...
per-channel teacher-substitution intervention on the S5 3v3 learners.

Change: dated entries covering the declaration, the execution-path confound, the run, and
scored predictions. The file stays local; the committed protocol is
`snowgym/training/reviews/m8_s8_declaration.md`.
...
M8-S5, 3v3 teacher imitation as the initializer, evaluated world-paired against S4.

Change: dated entries covering decisions, probes, the run, and gates. The file stays
local; the committed protocol is `snowgym/training/reviews/m8_s5_declaration.md`.

Upstream behavior: unchanged; documentation only.

## `refs/snowgym_m8_s4_dev_notes.md`

Reason: the user asked for a note on every development step. Local log for
M8-S4, 3v3 Engage metric definitions, teacher achievability and random-init floors.

Change: dated entries covering decisions, the run, and gates. The file stays
local; the committed protocol is `snowgym/training/reviews/m8_s4_declaration.md`.
